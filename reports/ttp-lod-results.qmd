...
```{r}
# ADD IN DAYS - now, I think we're too low...
coef_mams_30 <-
...
bind_rows(rename(mams_diff, regimen = key), 
          ...
          mams_diff_nl,
          remox_diff_nl) %>% 
  ggplot(aes(x = point.estimate.ratio, y = CI.width.ratio, shape = data)) +
  geom_point(#alpha = 0.9,
             aes(col = CI.width.ratio)) + 
  ...
  scale_color_gradient2(low = "darkgreen", mid = "green", high = "darkred") + 
  labs(x = TeX("$\\hat{\\theta}_{j, 30} / \\hat{\\theta}_{j, 42}$"),
       y = "95% CI width (30 day) / 95% CI width (42 day)") +
