...
```{r}
df_nc002_flow <- df_NC_002 %>% 
  filter(MBDY >= 0) %>% 
  dplyr::select(patient.id = USUBJID, MBDY, MBSTRESN, ACTARM) %>% 
  distinct() %>% 
...
df_nc005_flow <- df_NC_005 %>% 
  ...
  # Setting NA values to 42 for plotting purposes
  mutate(weeks_rounded = floor(MBDY/7),
         dtp_42 = ifelse(MBSTRESC == "NEGATIVE", 42, MBSTRESN)) %>%
...
df_st29_flow <- df_TBTC_S29x %>% 
  filter(MBDY >= 0) %>% 
  dplyr::select(patient.id = USUBJID, MBDY, MBSTRESN, MBSTRESC, MBTSTDTL) %>% 
  distinct() %>% 
  # Taking the first 8 weeks of observation
  filter(MBDY <= 7*8 & MBDY >= 0) %>%
  # Setting NA values to 42 for plotting purposes
...
alluv_st29 <- ggplot(filter(df_st29_flow, weeks_rounded %in% c(0,2,4,6)), aes(x = weeks_rounded, stratum = category, alluvium = patient.id, fill = category)) + 
  geom_flow() + 
  scale_fill_manual(values = c("darkgray",ucsf_col_pal[c(2,9)],"lightgray")) +
...
```{r}
(alluv_remox_2 + alluv_mams_2) / (alluv_nc002 + alluv_nc005) / (alluv_nc006 + alluv_st29) + 
  plot_annotation(tag_levels = "A") + 
  plot_layout(guides = "collect") &
