...
bind_rows(mutate(coefs_mams, Data = "MAMS-TB"), 
  ...
  mutate(`Censoring limit` = paste0(lod, " days")) %>% 
...
coefs_remox %>% 
  # mutate(lod = c(rep(42, 3), rep(30,3), rep(25,3))) %>% 
  mutate(CI.width = abs(.upper - .lower)) %>% 
  group_by(key) %>% 
...
## MAMS
```{r}
mams_diff <- bind_rows(mutate(df_42m$output, LOQ = "42 days"),
...
## REMoxTb
```{r}
remox_diff <- bind_rows(mutate(df_42r$output, LOQ = "42 days"),
