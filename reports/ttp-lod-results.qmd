...
load(here("data", "model-generated", "2024-03-26_linear-nc002-lod-42.RData"))
load(here("data", "model-generated", "2024-03-26_linear-nc002-lod-30.RData"))
load(here("data", "model-generated", "2024-03-26_linear-nc002-lod-25.RData"))
load(here("data", "model-generated", "2024-02-16_linear-nc006-lod-42.RData"))
load(here("data", "model-generated", "2024-02-16_linear-nc006-lod-30.RData"))
load(here("data", "model-generated", "2024-02-16_linear-nc006-lod-25.RData"))
load(here("data", "model-generated", "2024-02-16_linear-s29-lod-42.RData"))
load(here("data", "model-generated", "2024-02-16_linear-s29-lod-30.RData"))
load(here("data", "model-generated", "2024-02-16_linear-s29-lod-25.RData"))
load(here("data", "model-generated", "2024-02-16_linear-s29x-lod-42.RData"))
load(here("data", "model-generated", "2024-02-16_linear-s29x-lod-30.RData"))
load(here("data", "model-generated", "2024-02-16_linear-s29x-lod-25.RData"))
```
...
bind_rows(mutate(coefs_mams, Data = "MAMS-TB"), 
          ...
          mutate(coefs_nc002, Data = "NC-002 (M-Pa-Z)"),
...
# Investigation of Signal to Noise

## Replicates (REMoxTB + MAMS)

```{r}
df_replicates_remox <- df_analysis_remox %>% 
  group_by(trial_no, weeks) %>% 
  mutate(replicate = n_distinct(dtp_42)) %>% 
  filter(replicate > 1)

df_replicates_mams <- df_analysis_mams %>% 
  group_by(patient.id, weeks) %>% 
  mutate(replicate = n_distinct(dtp_42)) %>% 
  filter(replicate > 1)
```
...
```{r}
replicate_plot_fun <- function(df, time_var, participant_var, treatment_var, col_palette = ucsf_col_pal, ...){
  df %>% 
    dplyr::select(participant_var, time_var, treatment_var, dtp_42,censored_42, replicate) %>% 
    mutate(replicate = row_number()) %>% 
    pivot_wider(names_from = "replicate",
                values_from = "dtp_42") %>% 
    mutate(censored1 = ifelse(max(c(`1`,`2`),na.rm = TRUE) >= 42, 
                              "Above LOD = 42", "Below LOD = 42")#, 
           # ADD BACK IN THE 3RD REPLICATES ONCE BETTER PROCEDURE IS DETERMINED
           # censored2 = ifelse(max(c(`1`,`3`),na.rm = TRUE) >= 42, 
           #                    "Above LOD = 42", "Below LOD = 42")
           ) %>% 
    ggplot(aes(x = `1`)) +
    ...
    coord_equal() + 
    labs(x = TeX("DTP$_1$"), y = TeX("DTP$_2$")) + # TeX("DTP$_k$, $k = \\{2,3\\}")) + 
    theme(legend.position = "bottom",
          legend.title = element_blank())
}
p1 <- replicate_plot_fun(df_replicates_remox, time_var = "weeks", participant_var = "trial_no", treatment_var = "treat")
p2 <- replicate_plot_fun(df_replicates_mams, time_var = "weeks", participant_var = "patient.id", treatment_var = "Treatm_arm")
=======
p_remox_diff
```
...
ggsave(filename = here("graphs", "publication", "MAMS_REMOX-differentiation-regimens.png"),
       ...
       width = 14)
```
>>>>>>> 0aa3b386ed7820cb47d0236529730da0f1c09026
